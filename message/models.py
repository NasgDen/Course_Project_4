...
class Message(models.Model):
    ...
    owner = models.ForeignKey(
        CustomUser, on_delete=models.SET_NULL, related_name="message", verbose_name="Владелец", blank=True, null=True
    )
...
class Distribution(models.Model):
    ...
    datatime_first_sending = models.DateTimeField(
        verbose_name="Дата и время первой отправки", help_text="Дата и время первой отправки", null=True, blank=True
    )
    datetime_end_sending = models.DateTimeField(
    ...
    status = models.CharField(
        max_length=10, verbose_name="Статус", help_text="Статус", choices=STATUS_CHOICES, default="Создана"
    )
    message = models.ForeignKey(
        Message, on_delete=models.CASCADE, related_name="distributions", verbose_name="Сообщение"
    )
    ...
    mailings_status = models.BooleanField(
        default=False,
        verbose_name="Статус отключения рассылки",
    )
...
class AttemptedMailing(models.Model):
    ...
    attempt_datetime = models.DateTimeField(
        verbose_name="Дата и время попытки", help_text="Дата и время попытки", null=True, blank=True
    )
    status = models.CharField(
        max_length=20, verbose_name="Статус", help_text="Статус", choices=STATUS_CHOICES, default=""
    )
    mail_server_response = models.TextField(
        verbose_name="Ответ почтового сервера", help_text="Ответ почтового сервера"
    )
    distribution = models.ForeignKey(
        Distribution, on_delete=models.CASCADE, related_name="attempted_mailing", verbose_name="Рассылка"
    )
